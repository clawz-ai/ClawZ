...
**🖥️ 跨平台桌面应用**<br/>
基于 Tauri + Rust 构建，原生性能。已支持 macOS 和 Linux；Windows 支持计划中。
...
> 正式稳定版默认都是 notarized。只有在 notarization 尚未完成但必须紧急修复时，才通过 GitHub prerelease 提供应急版本，具体说明见对应 release notes。
...
**环境要求：** [Node.js](https://nodejs.org/) `22.22.0`（见 [`.nvmrc`](./.nvmrc)）、[pnpm](https://pnpm.io/) >= 10、[Rust](https://rustup.rs/) >= 1.77、[Tauri 依赖](https://v2.tauri.app/start/prerequisites/)
